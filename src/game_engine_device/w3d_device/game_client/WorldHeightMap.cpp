...
*/
WorldHeightMap::WorldHeightMap() : m_width(0), m_height(0), m_dataSize(0), m_data(NULL), m_cellFlipState(NULL),
 ...
 *
 */
WorldHeightMap::WorldHeightMap(ChunkInputStream *pStrm, Bool logicalDataOnly) : m_width(0), m_height(0), m_dataSize(0), m_data(NULL), m_cellFlipState(NULL),
...
Int WorldHeightMap::countTiles(InputStream *pStr)
{
	...
	if (tileWidth >= 10 && tileHeight >= 10)
		return (100);
	if (tileWidth >= 9 && tileHeight >= 9)
		return (81);
	if (tileWidth >= 8 && tileHeight >= 8)
		return (64);
	if (tileWidth >= 7 && tileHeight >= 7)
		return (49);
	if (tileWidth >= 6 && tileHeight >= 6)
		return (36);
	if (tileWidth >= 5 && tileHeight >= 5)
		return (25);
...
Bool WorldHeightMap::readTiles(InputStream *pStr, TileData **tiles, Int numRows)
{
	...
	for (row = 0; row < numRows * TILE_PIXEL_EXTENT; row++)
	{
		for (column = 0; column < hdr.imageWidth; column++)
		{
			...
			if (column >= (numRows * TILE_PIXEL_EXTENT))
				continue;
			r = buf[2];
			g = buf[1];
			b = buf[0];
...
Int WorldHeightMap::updateTileTexturePositions(Int *edgeHeight)
{
	...
	for (texClass = 0; texClass < m_numEdgeTextureClasses; texClass++)
	{
		...
		Int classHeight = yOrigin + width * TILE_PIXEL_EXTENT + TILE_OFFSET / 2;
		if (maxEdgeHeight < classHeight)
			maxEdgeHeight = classHeight;
...
#define DO_OLD_UV
		...
		if (ndx < this->m_dataSize - m_width - 1)
		{
			...
			if (texClass >= m_numTextureClasses)
				return false;
