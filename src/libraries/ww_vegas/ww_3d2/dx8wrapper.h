...
WWINLINE unsigned int DX8Wrapper::Convert_Color(const Vector3& color,float alpha)
{
	...
	// Multiply r, g, b and a components (0.0,...,1.0) by 255 and convert to integer. Or the integer values together
	// such that 32 bit integer has AAAAAAAARRRRRRRRGGGGGGGGBBBBBBBB.
	
	// Cross-platform C++ implementation replacing inline assembly
	int r = static_cast<int>(color.X * scale + 0.5f);
	int g = static_cast<int>(color.Y * scale + 0.5f);
	int b = static_cast<int>(color.Z * scale + 0.5f);
	int a = static_cast<int>(alpha * scale + 0.5f);
	...
	col = (static_cast<unsigned int>(a) << 24) | 
